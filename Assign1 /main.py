...
def test(app, search_method, check_optimality=False):
    app.agent = search_method(app.gw)
    goal = None
    for i, t in enumerate(app.gw.tiles):
        if t == gridworld.TILE_GOAL:
            goal = i
    ...
    if goal:
        # Set the agent to testing mode
        app.agent.set_testmode(True)

        for i in range(5):
            path = app.agent.start_agent()
            if not path:
                path = app.agent.path
            if not test_path(app, path, goal, check_optimality):
                rval = False
                break

        # Return the agent to normal mode
        app.agent.set_testmode(False)
        app.agent.init_info(app.agent_info)
        return rval


def test_path(app, path, goal, check_optimality=False):
    if not path or path[-1] != goal:
        print("Didn't find the goal")
        return False

    for i in range(1, len(path)):
        if path[i] not in app.gw.immtileneighbours(path[i - 1]):
            print("Not a valid path")
            return False
        if path[i] in path[:i]:
            print("Path contains a loop")
            return False

    if check_optimality:
        astar_path = astar.find_path(app.gw.immtileneighbours,
                                     path[0],
                                     goal,
                                     lambda tile: 1,
                                     lambda tile: not app.gw.tileblocked(*app.gw.indextopos(tile)),
                                     lambda a, b: astar.manhattan_dist(app.gw.indextopos(a),
                                                                       app.gw.indextopos(b)))
        if len(path) != len(astar_path):
            return False
    return True
...
if __name__ == "__main__":
    ...
    if nargs == 1:
        ...
        for w in sys.argv[2:]:
            ...
            app.openworld(w)
            if search_method == "d":
                rval = test(app, DFSAgent.DFSAgent)
            elif search_method == "i":
                rval = test(app, DFSAgent.IdDfsAgent, True)
            else:
                print("Incorrect search method specification")
        ...
        print("Usage: main.py [d|i] <world file>")
